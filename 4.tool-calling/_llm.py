...
WHICH PROVIDER GETS USED (in order)
    ...
    2. else if RADAR key + base-url env vars are set      -> RADAR qwen3-30b (unlimited)
...
# The base URL is a PRIVATE KCL endpoint, so it is NOT hardcoded here — it is
# read from RADAR_OPEN_MODEL_BASE_URL in the gitignored .env, keeping it out of
# git. Set that var in the .env next to the lessons (see README).
_RADAR = {
...
def _resolve() -> tuple[str, dict]:
    ...
    # then Gemini, then GitHub Models. RADAR needs BOTH its key and its base URL
    # (the URL lives only in .env), so skip it if either is missing.
    if os.environ.get(_RADAR["key_env"]) and _RADAR["base_url"]:
        return "radar", _RADAR
...
def _client_kwargs(http_client=None) -> dict:
    ...
    if not _CFG["base_url"]:
        # Only RADAR reads its base_url from the environment, so an empty one
        # means RADAR_OPEN_MODEL_BASE_URL is missing from the .env file.
        raise SystemExit(
            f"No base_url for provider '{PROVIDER}'.\n"
            f"Set RADAR_OPEN_MODEL_BASE_URL in the .env file next to the lessons."
        )
...
def build_client(http_client=None) -> OpenAI:
    """Sync client (p0001-p0003). Use this unless you specifically need async."""
    return OpenAI(**_client_kwargs(http_client))


def build_async_client(http_client=None) -> AsyncOpenAI:
    """Async client (AsyncOpenAI) — the real-world agent shape. See demo_async_agent.py."""
